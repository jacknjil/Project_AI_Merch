...
Real regression fixtures built from row 326's actual measured dimensions (canvas 1024×1024, `artBox` `{left:153, top:147, width:726, height:733}` on the original art; `{left:242, top:238, width:546, height:550}` after the 0.75 center-anchor shrink — verified directly against the real downloaded asset, not hand-predicted), per the fixture-fidelity lesson from the color-strategy Task 4 fix round. The arc-fit math depends only on canvas/`artBox` geometry and phrase string content, not on the art's actual pixel colors, so a synthetic transparent-canvas-with-opaque-circle fixture reproducing these exact measurements is sufficient — no need to check a 1MB+ real production PNG into the repo as a fixture, matching this codebase's existing lightweight-fixture convention (e.g. `getArtBoundingBox`'s own tests).

Empirically confirmed before writing the implementation plan: **row 326's primary phrase (`"Nature's Little Necromancer"`) fails to arc-fit on the original geometry (`ArcTextTooSmallError`, would render below 32px) but succeeds after the shrink retry alone** (dims 537×147 at the shrunk geometry). This means row 326 itself is a **shrink-alone-resolves-both** case, not a straight-line-fallback case — the straight-line fallback needs a separate, more extreme synthetic fixture to exercise at all.

- **The exact row 326 case (shrink-alone-resolves-both):** canvas/`artBox` geometry above, primary phrase `"Nature's Little Necromancer"` fails on the original geometry, secondary `"Reader of the Woods"` succeeds on the original geometry — confirm the retry fires anyway (not skipped due to secondary's early success) and both phrases apply after the shrink retry, with no straight-line fallback needed (`primaryUsedFallback`/`secondaryUsedFallback` both `false`). This is the fixture that directly proves the reported bug is fixed.
- **Both-fail-even-after-shrink case:** an extreme synthetic fixture (very long primary and secondary strings, e.g. 60+ characters each) on the same geometry, chosen to still fail `MIN_ARC_FONT_SIZE` even after the shrink retry — confirms the straight-line last-resort path fires for both and their `*UsedFallback` flags are `true`.
